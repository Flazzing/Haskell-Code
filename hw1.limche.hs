...
-- | A function to obtain the value of a binary tree's root value
getVal :: Tree -> Int
getVal (Leaf x)     = x
getVal (Node x _ _) = x
...
--   >>> isBST (Leaf 3)
--   True
--   isBST (Node 20(Leaf 10)(Leaf 30 (Leaf 5)(Leaf 40)))
...
isBST :: Tree -> Bool
isBST (Leaf x) = True
isBST (Node x l r) = (isBST l) && (getVal l < x) && (x < getVal r) && (isBST r)
...
--
inBST :: Int -> Tree -> Bool
inBST val (Leaf x) = val == x
inBST val (Node x l r) = val == x || inBST val l || inBST val r
